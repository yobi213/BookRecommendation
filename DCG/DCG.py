    ...
    def make_trfm(self, keyword, mode):
        ...
        for col in self.collection.find({find_tag: {'$exists': 'true'}}):
            ...
            try:
                ...
                elif(mode == 2): TRFM = col['RFM'] * t_value
                elif (mode == 3): TRFM = col['RFM'] ** t_value
                elif (mode == 4): TRFM = col['RFM'] ** (math.log(t_value, 2))
            except ValueError:
                print(t_value)
                print("There is no t-vlaue")
                return
...
#DCG
    ...
    def display_save_models(self, recommend_list, rel_list, mode, keyword):  # 추천 책들을 출력
        rfm_total = 0
        ...
        dcg_val = self.calc_DCG(rel_list, recommend_list)
        var_trfm = self.var_TRFM(recommend_list)

        self.save_data(dcg_val, rfm_total, var_trfm, mode, keyword)
        print('=' * 50)
        print("DCG: " + str(dcg_val))
        print("Total RFM: " + str(rfm_total))
        print("Var(TRFM): " + str(var_trfm))


    def save_data(self, DCG_value, rfm_total, var_TRFM, mode, keyword):
        print(keyword)
        if isinstance(keyword, unicode):
            keyword = str(keyword.encode('utf-8'))

        with open('./dic_file.csv', 'a') as csvfile:
            fieldnames = ['words', 'mode', 'DCG', 'Total_RFM', 'Var(TRFM)']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writerow({'words': keyword, 'mode': mode, 'DCG': DCG_value,
                             'Total_RFM': rfm_total, 'Var(TRFM)': var_TRFM})


    def run_test(self, keyword, mode, recommend_list, rel_list):
        new_list = self.make_trfm(keyword, mode)
        merged_list = self.merge_trfm(recommend_list, new_list)
        sorted_list = self.sort_trfm(merged_list)

        new_rel_list = self.make_dcg(keyword)
        merged_rel_list = self.merge_rel(rel_list, new_rel_list)
        # dcg_score = tt.calc_DCG(merged_rel_list, sorted_list)

        # tt.display_books(sorted_list, merged_rel_list)
        print("*"*10 + " Mode :" + str(mode) +" "+ "*"*10)
        self.display_save_models(sorted_list, merged_rel_list, mode, keyword)
